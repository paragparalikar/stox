        ...
        protected void onProposedChange(final List<Node> newNodes, int[] toBeRemoved) {
            ...
            for (int i = 0; i < toBeRemoved.length; i += 2) {
                for (int j = toBeRemoved[i]; j < toBeRemoved[i + 1]; j++) {
                	System.out.println("Removing element index : "+j);
                	final Node node = children.get(j);
                	System.out.println("Node at index is : "+node);
                	System.out.println("childSet contains node ? "+childSet.contains(node));
                	childSet.forEach(n -> System.out.println(n));
                	
                    childSet.remove(children.get(j));
                    System.out.println("Removed "+j+" from childset");
            ...
            try {
                ...
                System.out.println("childSet size : "+childSet.size()+", newNodes size : "+newNodes.size()+", newLength : "+newLength);
